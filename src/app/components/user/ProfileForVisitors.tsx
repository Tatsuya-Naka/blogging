...
export default function ProfileForVisitors({ userData }: Props) {
    ...
    const { data: profile, refetch: refetchProfile } = trpc.topic.getAccountProfile.useQuery({
        userId: userId,
    });
    ...
    const items = profile?.length;

    const {data: user, refetch: refetchUser} = trpc.demo.getUserInfo.useQuery({
        userId,
    });
